...
return new class extends Migration
{
    public function up(): void
    {
        Schema::create('deliveries', function (Blueprint $table) {
            $table->id(); $table->string('number', 30)->nullable()->unique();
            $table->foreignId('worker_id')->constrained()->restrictOnDelete();
            $table->date('delivery_date')->index(); $table->string('reason', 180)->nullable(); $table->text('notes')->nullable();
            ...
            $table->timestamp('confirmed_at')->nullable(); $table->timestamp('annulled_at')->nullable(); $table->string('annul_reason', 500)->nullable(); $table->timestamps();
        });
        Schema::create('delivery_items', function (Blueprint $table) {
            $table->id(); $table->foreignId('delivery_id')->constrained()->cascadeOnDelete();
            $table->foreignId('product_variant_id')->constrained()->restrictOnDelete();
            $table->decimal('quantity', 14, 3); $table->string('notes', 500)->nullable(); $table->timestamps();
    ...
    public function down(): void
    {
        ...
        Schema::dropIfExists('delivery_serialized_items'); Schema::dropIfExists('delivery_items'); Schema::dropIfExists('deliveries');
