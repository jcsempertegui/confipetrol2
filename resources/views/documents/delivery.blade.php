...
<table><thead><tr><th>#</th><th>Producto</th><th>SKU / variante</th><th>Series asignadas</th><th class="right">Cantidad</th></tr></thead><tbody>@foreach($delivery->items as $item)<tr><td>{{ $loop->iteration }}</td><td>{{ $item->variant->product->name }}</td><td>{{ $item->variant->sku }}<br><span class="muted">{{ $item->variant->name }}</span></td><td>{{ $item->serializedItems->pluck('serial_number')->join(', ') ?: '—' }}</td><td class="right">{{ number_format((float)$item->quantity,3) }}</td></tr>@endforeach</tbody></table>
