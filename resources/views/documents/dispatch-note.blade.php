...
<table><thead><tr><th>#</th><th>Producto</th><th>SKU / variante</th><th>Series</th><th class="right">Cantidad</th></tr></thead><tbody>@foreach($dispatchNote->items as $item)<tr><td>{{ $loop->iteration }}</td><td>{{ $item->variant->product->name }}</td><td>{{ $item->variant->sku }}<br><span class="muted">{{ $item->variant->name }}</span></td><td>{{ $item->serializedItems->pluck('serial_number')->join(', ') ?: '—' }}</td><td class="right">{{ \App\Support\Quantity::format($item->quantity) }}</td></tr>@endforeach</tbody></table>
