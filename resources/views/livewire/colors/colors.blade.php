    ...
    <div class="row align-items-center mb-3 px-2">
        <div class="col-12 d-flex justify-content-between align-items-center">
            <ol class="breadcrumb mb-0 d-flex align-items-center">
                ...
                <li class="breadcrumb-item" style="font-weight: 500; font-size: 18px;">Colores</li>
            </ol>
            @can('crear-colores')
    ...
    <div class="card">
        <div class="card-body px-4 mt-2">
            <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-2 mb-3">
                <h5 class="card-title mb-0">Listar Colores</h5>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="d-flex order-actions">
                                            @can('editar-colores')
                                            <a href="javascript:;" wire:click="edit({{ $color->id }})"
                                                data-bs-toggle="modal" data-bs-target="#theModal" class="btn-action-primary"><i
                                                    class="bx bxs-edit-alt"></i></a>
                                            @endcan
                                            @if ($color->status == 1)
                                                @can('eliminar-colores')
                                                    <a href="javascript:;" onclick="confirmDeleteColor({{ $color->id }}, 'delete')"
                                                        class="btn-action-danger ms-1"><i class="bx bxs-trash"></i></a>
                                                @endcan
                                            @else
                                                @can('restaurar-colores')
                                                    <a href="javascript:;" onclick="confirmDeleteColor({{ $color->id }}, 'restore')"
                    ...
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="theModalLabel">
                                ...
                                {{ $isEditMode ? 'ACTUALIZAR COLORES' : 'REGISTRAR COLORES' }}
                            </h5>
...
<script>
    document.addEventListener('livewire:init', function () {
        Livewire.on('colorStoreOrUpdate', (Msg) => {
            $('#theModal').modal('hide');
            toast(Msg, 'success')
        });

        Livewire.on('colorDeleted', (Msg) => {
            toast(Msg, 'success')
        });
    });

    function confirmDeleteColor(id, action) {
        Swal.fire({
