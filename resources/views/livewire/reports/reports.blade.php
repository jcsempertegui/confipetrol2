...
<div class="card">
    ...
    @if($reportType === 'stock')
        <thead><tr><th>Categoría</th><th>Producto</th><th>SKU / variante</th><th>Unidad</th><th>Control</th><th class="text-end">Stock</th><th class="text-end">Mínimo</th><th>Estado</th></tr></thead>
        <tbody>@forelse($rows as $r)@php $stock=(float)($r->stock??0);$label=$this->stockLabel($stock,(float)$r->minimum_stock);@endphp<tr><td>{{ $r->product->category->name }}</td><td><strong>{{ $r->product->name }}</strong><div class="small text-muted">{{ $r->product->code }}</div></td><td>{{ $r->sku }}<div class="small text-muted">{{ $r->name }}</div></td><td>{{ $r->product->unit }}</td><td>{{ $r->product->tracking_type==='serialized'?'Por serie':'Por cantidad' }}</td><td class="text-end fw-bold">{{ number_format($stock,3) }}</td><td class="text-end">{{ number_format((float)$r->minimum_stock,3) }}</td><td><span class="badge bg-{{ $label==='Agotado'?'dark':($label==='Stock bajo'?'danger':'success') }}">{{ $label }}</span></td></tr>@empty<tr><td colspan="8" class="text-center py-5 text-muted">No existen productos que coincidan con los filtros.</td></tr>@endforelse</tbody>
