...
beforeEach(function () {
    ...
    $this->user=User::factory()->create(); $this->user->assignRole($role); $this->actingAs($this->user);
...
it('shows stock and a kardex with running balances', function () {
    ...
    Livewire::test(InventoryController::class)->assertSee('Guantes')->assertSee('7.000')
        ->call('viewKardex',$this->variant->id)->assertSee('ING001'.now()->format('dmy'))->assertSee('SAL001'.now()->format('dmy'))
        ->assertSee('10.000')->assertSee('3.000')->assertSee('7.000');
});

it('protects kardex independently from stock visibility', function () {
    $role=Role::create(['name'=>'SOLO STOCK','guard_name'=>'web']); $role->givePermissionTo('ver-inventario');
    $user=User::factory()->create(); $user->assignRole($role);
